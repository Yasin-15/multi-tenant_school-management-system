    ...
    <>
      ...
      <div className={`sidebar ${isOpen ? 'open' : ''}`}>
        <div className="p-6 border-b border-slate-700">
          ...
          <p className="text-sm text-slate-400 mt-1">{user?.role?.toUpperCase()}</p>
        </div>
              ...
              if (link.submenu) {
                return (
                  <li key={index} className="submenu">
                    <span className="flex items-center px-6 py-3 text-slate-300 hover:text-white transition-colors cursor-pointer">
                      <Icon className="w-5 h-5 mr-3" />
                      {link.label}
                    </span>
                    <ul className="bg-slate-800/50">
                      {link.submenu.map((subItem) => (
                        <li key={subItem.path}>
                          <Link
                            to={subItem.path}
                            onClick={closeSidebar}
                            className={`flex items-center pl-14 pr-6 py-2 hover:bg-slate-800 transition-colors ${location.pathname === subItem.path ? 'bg-slate-800 text-blue-400' : 'text-slate-400'
                              }`}
                          >
              ...
              return (
                <li key={link.path}>
                  <Link
                    to={link.path}
                    onClick={closeSidebar}
                    className={`flex items-center px-6 py-3 hover:bg-slate-800 transition-colors ${isActive ? 'bg-slate-800 border-l-4 border-blue-500' : ''
                      }`}
                  >
                    <Icon className="w-5 h-5 mr-3" />
